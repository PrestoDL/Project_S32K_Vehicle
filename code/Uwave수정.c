...
#define NVIC_IPR49		*((volatile unsigned char*)(NVIC_IPR_BASE + 0x31))
...
#define PORTA_PCR6  *((volatile unsigned*)(PORTA_BASE + 0x18)) // uWave
...
////////////////////////////////////////////
void uWave_Sensor(void)
{
    if (GEARSET == R || GEARSET == D)
    {
        if (uWaveCount == 0)
            GPIOA_PSOR |= (1 << PTA8);
        ...
        uWaveCount++;
        if (uWaveCount >= 6000) // 10us * 6000 = 60ms
        {
            uWaveCount = 0;
            uWaveDistance = uWaveLevelHigh;
            uWaveLevelHigh = 0;
        }
    }
}
...
//		FTM2_C2V = (velocity*1200000)/4095; ////////////////// 숫자값 추후 확인
		FTM2_C2V = (velocity*1200000)/4095;
		FTM2_C5V = 0; ////////////////// 숫자값 추후 확인
		//GEARSET = D;
...
//		FTM2_C5V = (velocity*1200000)/4095; ////////////////// 숫자값 추후 확인
		FTM2_C5V = (velocity*1200000)/4095;
		FTM2_C2V = 0; ////////////////// 숫자값 추후 확인
		//GEARSET = R;
	}
	else
	{}
...
int main(void) {
   ...
   for(;;) { // step delay 0.01
      ...
      if (lastStepTime < time) {
         if ((int)uWaveDistance*0.17 < 11) { // uWave detect
            brakeLED = 1;
            SelectPiezoGear = 10;

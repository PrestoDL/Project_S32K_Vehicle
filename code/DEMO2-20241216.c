...
#define PCC_FTM2    *((volatile unsigned*)(PCC_BASE + 0xE8)) //////////////////// 20241211 한구 추가
...
#define NVIC_IPR48		*((volatile unsigned char*)(NVIC_IPR_BASE + 0x30))
#define NVIC_IPR59		*((volatile unsigned char*)(NVIC_IPR_BASE + 0x3B)) /////////////// 추가 한구
...
#define FTM0_MOD	*((volatile unsigned*)(FTM0_BASE + 0x8))		//////////////////////// 추가 한구
...
#define FTM2_BASE	(0x4003A000)								//////////////////////20241210
...
#define FTM2_C5SC  	*((volatile unsigned*)(FTM2_BASE + 0x34))		//////////////////////// 추가 한구
#define FTM2_C5V   	*((volatile unsigned*)(FTM2_BASE + 0x38))		//////////////////////// 추가 한구
#define FTM2_CNTIN 	*((volatile unsigned*)(FTM0_BASE + 0x4C))

#define PWMEN0_BIT	16/////////////////////////////////////////////////////// 추가 한구
#define PWMEN1_BIT	17
#define PWMEN2_BIT	18
#define PWMEN5_BIT  21 ///////////////////////////////////////////////// 20241210
#define CLKS_BITS	3
#define PS_BITS		0
#define MSB_BIT     5
#define MSA_BIT     4
...
#define ADC0_SE12   12	// PTB3 VR&CDS 제대로 동작하지 않을 경우, 메인보드 위 가변 저항으로 동작 수정.



#define PORTA_BASE	(0x40049000)
#define PORTA_PCR1	*((volatile unsigned*)(PORTA_BASE + 0x4)) // button_1 : Gear(Forward / Backward) ///////////////// 한구 사용.
...
#define PORTA_PCR6  *((volatile unsigned*)(PORTC_BASE + 0x18)) // uWave : trig
#define PORTA_PCR7  *((volatile unsigned*)(PORTC_BASE + 0x1C)) // uWave : echo
...
#define PORTB_PCR3	*((volatile unsigned*)(PORTB_BASE + 0xC)) // VR&CDS, if 제대로 동작 않을 시, 사용X 하고 메인보드 가변저항으로 대체
...
//#define PORTC_PCR0  *((volatile unsigned*)(PORTC_BASE + 0x0)) // DC motor // 사용 안 할 예정
//#define PORTC_PCR1  *((volatile unsigned*)(PORTC_BASE + 0x4)) // Servo motor // 사용 안 할 예정
//#define PORTC_PCR2  *((volatile unsigned*)(PORTC_BASE + 0x8)) // Step motor // 사용 안 할 예정
#define PORTC_PCR3  *((volatile unsigned*)(PORTC_BASE + 0xC)) // Buzzer
...
#define PORTD_PCR16 *((volatile unsigned*)(PORTD_BASE + 0x40)) // Servo Motor
...
#define MUX_BITS	8
#define IRQC_BITS	16 //Interrupt Type  /////////////////// 추가 한구
#define ISF_BIT		24 //Interrupt set flag //////////////// 추가 한구
...
#define PTB1    1
#define PTB2    2
#define PTB3    3	//VR&CDS 용, 만약 제대로 동작하지 않는다면 메인 보드 가변저항으로 대체
#define PTB4	4

//#define PTC0    0 // 사용 안 할 예정
//#define PTC1    1 // 사용 안 할 예정
//#define PTC2    2 // 사용 안 할 예정
//#define PTC3    3 // 사용 안 할 예정
...
#define PTD14	14 /////////// Servo Motor PWM 추가
#define PTD15   15 /////////// DC Motor(D) PWM 추가
#define PTD16   16 /////////// DC Motor(R) PWM 추가
...
#define MAXSPD	160 // can be modified
#define UWAVELIMIT 15 // uWave detect distance, cm

int time = 0; // unit : 0.00001s
int lastStepTime = 0; // for checking time difference
int uWaveEcho = 0; // for checking uWave echo time
float velocity = 0; // 0.00000 ~
float uWaveDistance = 0; // 0.00000 ~

int handleAngle; // 0 ~ 2047 ~ 4095
int blinkerSwitch = 1; // 0, 1, 2 : left, idle, right
int blinkerTime; // 0 ~
int wiperSwitch = 0; // 0, 1 : wiper off, wiper on
int uWaveDetect = 0; // 0, 1 : not detected, detected
int brakeLED = 0; // 0, 1 : no brake, brake

int SelectGearForwardBackward = 1; // 0, 1 : Backward, Forward  ////////// 추가 한구
int SelectPiezoGear = 0;
...
#define PTC13	13//Parking
volatile uint8_t GEARSET = P;
#define PTD16	16//servo ftm0 ch 1
...
void PORT_init() {
   ...
   PORTA_PCR1 &= ~((0b111) << MUX_BITS);///////////////////////////////////// 한구 사용 예정
   PORTA_PCR1 |= (1 << MUX_BITS);//////////////////////////////////////////// 한구 사용 예정

   PORTA_PCR1 &= ~((0b1111)<<IRQC_BITS); /////////////////////////////////// 한구 추가
   PORTA_PCR1 |= ((0b1001)<<IRQC_BITS); /////////////////////////////////// 한구 추가
   ...
   PORTA_PCR6 &= ~((0b111) << MUX_BITS);
   PORTA_PCR6 |= (1 << MUX_BITS);
   PORTA_PCR7 &= ~((0b111) << MUX_BITS);
   PORTA_PCR7 |= (1 << MUX_BITS);

   GPIOA_PDDR |= (1 << PTA6); // output
   GPIOA_PDDR &= ~(1 << PTA1) & ~(1 << PTA2) & ~(1 << PTA3) & ~(1 << PTA7) & ~(1 << PTA11) & ~(1 << PTA12); // input
   ...
   PORTB_PCR3 &= ~((0b111)<<MUX_BITS);	////////////////////// VR&CDS 용, 만약 메인보드 가변 저항 사용시, 주석처리
   PORTB_PCR3 |= ((0b010)<<MUX_BITS);	////////////////////// VR&CDS 용, 만약 메인보드 가변 저항 사용시, 주석처리
...
void set7segmentN(){
	GPIOD_PSOR |= (1 << PTD2) | (1 << PTD4) | (1 << PTD6);
	GPIOD_PCOR |= (1 << PTD0) | (1 << PTD7) | (1 << PTD3) | (1 << PTD5);
}
...
void LPIT0_init(void) {
   ...
   LPIT_TVAL0 = 400; // 0.00001s (e-5)

   LPIT_TCTRL0 &= ~((0b11) << MODE_BITS);
   LPIT_TCTRL0 |= (1 << T_EN_BIT);
}

void LPIT0_disable(void) {
   LPIT_MCR &= ~(1 << M_CEN_BIT);
}

void LPIT0_enable(void) {
   LPIT_MCR |= (1 << M_CEN_BIT);
}

void NVIC_init_IRQs(void) {
   NVIC_ICPR1 |= (1 << (48 % 32)) | (1 << (59%32));  ////////// 추가 한구
   NVIC_ISER1 |= (1 << (48 % 32)) | (1 << (59%32));  ///////// 추가 한구
   NVIC_ICPR1 |= (1 << (61 % 32));
   NVIC_ISER1 |= (1 << (61 % 32));
   NVIC_IPR48 = (10<<4);
   NVIC_IPR59 = (8<<4);////////////////////////////// 추가 한구 : Interrupt PortA
   NVIC_IPR61 = (8<<4);
...
void PORTA_IRQHandler(void) // PORTA interrupt handler
{
	...
	else if(GEARSET == D && velocity == 0)
	{
		...
		GEARSET = R;
		SelectPiezoGear = 1;
		SelectGearForwardBackward = 0;// // 0, 1 : Backward, Forward
	}
...
void adc_start(void) {
   ...
   // ADC0_SC1A |= (ADC0_SE12 << ADCH_BITS); // VR&CDS 제대로 동작하지 않을 시, 메인보드 위 가변 저항으로 대체.
}
...
void setHandle(int handleAngle) {
    if(handleAngle > 3072)
    {
    	...
    	FTM0_C1V = 13200-1200; // VR&CDS용
    	//FTM2_C5V = 6000-1200; // 메인보드 가변저항용
    }
    else if(handleAngle > 1024)
    {
  	  FTM0_C1V = 9600-1200;
    }
    else if(handleAngle > -1)
    {
    	if(velocity > 80){
    		SelectPiezoGear = 1;
    		FTM0_C1V = 9600-1200;
...
void setBlinkLED(int blinkerSwitch) {
   ...
   else if ((time - blinkerTime) % 50000 < 25000) { // turn on
      if (blinkerSwitch == 0) {
...
void setDCmotorGear(int velocity) {
	if(SelectGearForwardBackward == 1) {// 전진
		FTM2_C5V = 0;
		FTM2_C2V = (velocity*1200000)/4095; ////////////////// 숫자값 추후 확인
		//GEARSET = D;

	}
	else if (SelectGearForwardBackward == 0) {
		FTM2_C2V = 0;
		FTM2_C5V = (velocity*1200000)/4095; ////////////////// 숫자값 추후 확인
		//GEARSET = R;
	}
	else
	{}
...
int main(void) {
   ...
   for(;;) { // step delay 0.00001

      // checking handleAngle
      adc_start();
      handleAngle = read_adc_chx(); // 0 ~ 2047 ~ 4095

      setHandle(handleAngle);						// 한구 작업
      setDCmotorGear((int)velocity);				// 한구 작업
      setPiezoGear();
      ...
      if (lastStepTime < time) { // checking 0.00001s stepTime
         // setting uWave sensor
         if ((time % 1000) == 0) {
            GPIOA_PSOR |= (1 << PTA6);
         }
         ...
         if ((GPIOA_PDIR & (1 << PTA7)) == 1) { // detected
            uWaveEcho = time % 1000; // echo time, 0.00000 ~ 0.00999
            uWaveDistance = uWaveEcho * 1.7 // as 340m/s, velo : 0.17cm / 1step
            if (uWaveDistance < UWAVELIMIT) { // distance less then limit
               uWaveDetect = 1; // reset in 0.001step
            }
         }
         ...
         if ((time % 1000) == 0) { // checking 0.01s stepTime
            if (uWaveDetect == 1) { // uWave detect
               brakeLED = 1;
               SelectPiezoGear = 1;
               velocity -= 2; // -200 per sec
               uWaveDetect = 0;
            }
               ...
               brakeLED = 0;
               velocity -= velocity * 0.0001; // -1% per sec
            }
